...
def TF_U_net(int_height_data, int_width_data, int_dim_data, dropout_rate=0.2):
    ...
    output.add(
        tf.keras.layers.Conv2D(output_channels, 1, strides=1, kernel_initializer=tf.keras.initializers.GlorotNormal()))
...
def TF_binary_cross_entropy(output_map1, output_map2, y_, tf_class_weights):
    ...
    tf_loss1 = -tf.math.reduce_mean(
        temp_y_ * tf_class_weights[0] * tf.math.log(tf.clip_by_value(temp_output_map, tiny, 1 - tiny)))
    tf_loss2 = -tf.math.reduce_mean((bin_true - temp_y_) * tf_class_weights[1] * tf.math.log(
        bin_true - tf.clip_by_value(temp_output_map, tiny, 1 - tiny)))
...
def TF_cross_entropy_out_of_cluster(tf_activationmap, tf_region_of_interest, y_):
    ...
    loss1 = -tf.math.reduce_mean((bin_true - tf_region_of_interest) * tf.math.log(
        (bin_true - tf.clip_by_value(tf_activationmap, tiny, 1 - tiny))), [1, 2])
...
def loss(model, x, y, tf_region_of_interest, tf_alpha_mask, tf_class_weights, training, fileid=[], out_dir=[],
         folder=[], regionID=[], save=False):
    pred = model(x, training=training)

    tf_lambda1 = 1
    tf_prediction_pooling_target, tf_sum_activationmap_target, tf_prediction_pooling_target1, tf_prediction_pooling_target2 = TF_Soft_clipping_pooling_layer_experimental(
        pred[:, :, :, 0], tf_alpha_mask, tf_region_of_interest)
    tf_loss_target_segmentation = TF_binary_cross_entropy(tf_prediction_pooling_target1, tf_prediction_pooling_target2,
                                                          y[:, 0], tf_class_weights)
    ...
    if not training and save:
        save_pred(y, tf_region_of_interest, pred, fileid, out_dir, folder, regionID, tf_sum_activationmap_target,
                  tf_prediction_pooling_target)
...
def grad(model, x, y, z, tf_alpha_mask, tf_class_weights, epoch_auc=None, vali_auc=None):
    with tf.GradientTape() as tape:
        y_true, y_pred, pred, loss_value, tf_prediction_pooling_target, tf_sum_activationmap_target, tf_loss_target_segmentation, tf_loss_outside_target_region = loss(
            model, x, y, z, tf_alpha_mask, tf_class_weights, training=True)
    return y_true, y_pred, loss_value, tape.gradient(loss_value,
                                                     model.trainable_variables), tf_prediction_pooling_target, tf_sum_activationmap_target, tf_loss_target_segmentation, tf_loss_outside_target_region


def save_pred(y_true, mask, pred, fileid, out_dir, folder, regionID, tf_sum_activationmap_target,
              tf_prediction_pooling_target):
    if not os.path.exists(out_dir + "/vali/" + folder):
        os.mkdir(out_dir + "/vali/" + folder)

    for i in range(0, len(y_true)):
        cv2.imwrite(out_dir + "/vali/" + folder + "/" + str(fileid[i].decode("utf-8")) + "_predict_" + "label" + str(
            int(y_true[i])) + ".png", pred.numpy()[i, :, :, 0] * 255)
        cv2.imwrite(out_dir + "/vali/" + folder + "/" + str(fileid[i].decode("utf-8")) + "_true_" + "label" + str(
            int(y_true[i][0])) + ".png", mask[i, :, :] * 255)
        np.save(out_dir + "/vali/" + folder + "/" + str(fileid[i].decode("utf-8")) + "_predictval_" + "label" + str(
            int(y_true[i][0])) + ".npy",
                [y_true[i][0], tf_sum_activationmap_target.numpy()[i], tf_prediction_pooling_target.numpy()[i]])
...
def train_CSN(x_train: list,
    ...
    x_train, y_train, mask_train, fnames_train = make_data_divisable(x_train, y_train, mask_train, fnames_train,
                                                                     batch_size)
    ...
    train_dataset = tf.data.Dataset.from_generator(data_gen(x_train, y_train, mask_train, fnames_train, epochs),
                                                   (tf.float32, tf.float32, tf.float32, tf.string)).batch(
        batch_size)  # .prefetch(batch_size)
    vali_dataset = tf.data.Dataset.from_generator(data_gen(x_vali, y_vali, mask_vali, fnames_vali, epochs),
                                                  (tf.float32, tf.float32, tf.float32, tf.string)).batch(
        batch_size)  # .prefetch(batch_size)
    ...
    optimizer = tf.keras.optimizers.RMSprop(
        lr_scheduler(learning_rate, lr_scheduler_epochs * (len(x_train) // batch_size), lr_scheduler_factor,
                     initial_epoch * (len(x_train) // batch_size)), float_decay_a, momentum_RMSprop)
    ...
    for i in range(initial_epoch, epochs):
        ...
        for x, y, z, fileid in train_dataset.take(len(x_train) // batch_size).as_numpy_iterator():
            ...
            y_true, y_pred, loss_value, grads, prediction_pooling, sum_activationmap, loss_target_segm, loss_outside_region = grad(
                model, x, y, z, tf_alpha_mask, tf_class_weights, epoch_auc, vali_auc)
        ...
        print("Train: ", batch_counter, "/", number_of_batches_train, "Time:", np.around(end_time - start_time, 2),
              "Time per Step:", np.around((end_time - start_time) / number_of_batches_train, 2), "    TrainLoss: ",
              train_loss_result[-1].numpy(), "    TrainAUC: ", train_auc_results[-1].numpy())
        ...
        for x, y, z, fileid in vali_dataset.take(len(x_vali) // batch_size).as_numpy_iterator():
            ...
            y_true, y_pred, pred, loss_value, prediction_pooling, sum_activationmap, loss_target_segm, loss_outside_region = loss(
                model, x, y, z, tf_alpha_mask, tf_class_weights, training=False, fileid=fileid, out_dir=out_dir,
                folder="vali" + str(i), save=save_vali_images)
        ...
        print("Vali: ", batch_counter, "/", number_of_batches_vali, "Time:", np.around(end_time - start_time, 2),
              "Time per Step:", np.around((end_time - start_time) / number_of_batches_vali, 2), "    ValiLoss: ",
              vali_loss_result[-1].numpy(), "    Vali AUC: ", vali_auc_results[-1].numpy())
...
def predict_CSN(x_test: list,
    ...
    test_dataset = tf.data.Dataset.from_generator(data_gen(x_test, y_test, mask_test, fnames_test),
                                                  (tf.float32, tf.float32, tf.float32, tf.string)).batch(
        batch_size).prefetch(batch_size)
    ...
    for e in range(len(epochs_to_test)):
        ...
        for x, y, z, fileid in test_dataset.take(len(x_test) // batch_size).as_numpy_iterator():
            y_true, _, pred, _, tf_prediction_pooling_target, tf_sum_activationmap_target, _, _ = loss(model, x, y, z,
                                                                                                       tf_alpha_mask,
                                                                                                       tf_class_weights,
                                                                                                       training=False,
                                                                                                       save=False)
            ...
            for i in range(0, batch_size):
                cv2.imwrite(out_dir + "test_tiles/epoch" + str(epochs_to_test[e]) + "/" + str(
                    fileid[i].decode("utf-8")) + "_predict_label" + str(int(y_true[i][0])) + ".png",
                            pred.numpy()[i, :, :, 0] * 255)
                cv2.imwrite(out_dir + "test_tiles/epoch" + str(epochs_to_test[e]) + "/" + str(
                    fileid[i].decode("utf-8")) + "_true_label" + str(int(y_true[i][0])) + ".png", z[i, :, :] * 255)
                np.save(out_dir + "test_tiles/epoch" + str(epochs_to_test[e]) + "/" + str(
                    fileid[i].decode("utf-8")) + "_predictval_label" + str(int(y_true[i][0])) + ".npy",
                        [y_true[i][0], tf_sum_activationmap_target.numpy()[i], tf_prediction_pooling_target.numpy()[i]])
